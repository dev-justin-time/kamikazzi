...
export class LandscapingManager {
    ...
    applyTool(point, tool, targetHeight) {
        ...
        const bSize = parseFloat(document.getElementById('sg-brush-size-slider').value);
        const bStrength = parseFloat(document.getElementById('sg-brush-strength-slider').value);
        ...
        const stampHeight = parseFloat(document.getElementById('sg-stamp-height-slider')?.value || 0.8);
        const vDetail = parseInt(document.getElementById('sg-v-detail-slider')?.value || 2);
        const treeType = document.getElementById('sg-tree-type-select')?.value || 'pine';
        const cityType = document.getElementById('sg-city-type-select')?.value || 'rural';
            ...
            for (let i = startI; i <= endI; i += brushStep) {
                ...
                if (distSq < pixelRadius * pixelRadius) {
                    ...
                    if (tool === 'raise') {
                        ...
                        const vNoiseAmt = parseFloat(document.getElementById('sg-v-noise-slider')?.value || 0);
                        let noiseVal = 0;
                        if (vNoiseAmt > 0) {
                        ...
                        const bRange = parseFloat(document.getElementById('sg-biome-range-slider').value);
